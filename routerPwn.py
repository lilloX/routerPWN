...
parser = argparse.ArgumentParser(description = 'Two operational modes: Signle IP\IP List',usage = aux.banner(__version__),
                                 ...
                                 '\n\t'+script_name+' -l ip_list.txt -o test.out\n\nYou have to specify -i OR -l. Output use is optional',formatter_class=argparse.RawTextHelpFormatter)
		...
		for line in ip_list:
			...
			if opt.o_file:
				...
				if router.vulnerable:
					output.writelines(router.model+','+router.ip+','+router.port+','+str(router.vulnerable)+','+router.exploit+','+router.username+','+router.password+'\n')
				else:
					if router.model != '':
						output.writelines(router.model+','+router.ip+','+router.port+','+router.exploit+'\n')
			del router
	output.close()
	ip_list.close()
